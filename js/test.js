let questions = [
    {
        ...
        type: "multichoix", choix: ["oui","non","ne sait pas"]
    },
    {
        question: "quelle est votre température ?", 
        type: "text", min: 33, max: 42, unite: "°C"
    },
    {
    ...
    },
    {
        ...
        type: "text", min: 5, max: 120, unite: "ans"
    },
    {
        question: "Quel est votre poids ?", 
        type: "text", min: 20, max: 220, unite: "kg"
    },
    {
        question: "Quelle est votre taille ?", 
        type: "text", min: 80, max: 250, unite: "cm"
    },
    {
...
form.addEventListener("submit",(e) => {
    ...
    if(i == 24){
        main.innerHTML = `
                        ...
                        <h5> Résultats </h5>
                        <p class="title font-weight-normal fs-4">${resultatAlgo()}</p>
                        <hr>
...
// let btnChoix = document.getElementsByClassName('bclChoix');
// for(let l = 0;l<btnChoix.length;l++){ 
//     btnChoix[l].addEventListener("click", e => {
//         e.target.classList.toggle('choisi')
//     });
// }
let facteurPronosticAge = () => {
    if(r[12] < 15) return 'stop';
    return 'contunue';
}
let facteurGravite = () => {
    let f = 0
    if(r[0] == "oui" && (r[1] <= 35.4 || r[1] >= 39)) f++
    if(r[6] == "oui" && r[7] == "oui") f++
    if(r[8] == "oui") f++
    if(r[9] == "oui") f++
    if(r[10] == "mal" || r[10] == "très mal") f++
    if(f > 0){
        if(r[9] == "oui" && r[8] == "oui" && r[1] <= 35.4)
            return [f, "majeur"]
        else if(r[1] >= 39 && r[6] == "oui" && (r[10] == "mal" || r[10] == "très mal"))
            return [f, "mineur"]
    }
    return [f, null]
}
let facteurPronostic = () => {
    let p = 0
    if(r[12] >= 70 || (r[14] == "oui" || r[14] == "ne sait pas") || r[15] == "oui" || r[16] == "oui" || r[17] == "oui" 
        || r[18] == "oui" || r[19] == "oui" || r[20] == "oui" || r[21] == "oui" || r[22] == "oui" )
        p++
    return p
}
let resultatAlgo = () => {
    let nbrGravite = facteurGravite();
    let nbrProstic = facteurPronostic();
    let res = '';
    if(facteurPronosticAge() == 'stop')
        return `Prenez contact avec votre médecin généraliste au moindre doute. 
                Cette application n’est pour l’instant pas adaptée aux personnes de moins de 15 ans. En cas d’urgence, 
                appeler le 15.`;
    if((r[0] == 'oui') || (r[2] == 'oui' && r[4] == 'oui') || 
        (r[2] == 'oui' && r[3] == 'oui' || r[1] == 'oui' && r[5] == 'oui'))
    {
        if(nbrProstic == 0){
            if(nbrGravite[0] == 0 && r[12] < 50){
                res = ` 
            ...
            }else if((nbrGravite[0] == 0 && r[12] >= 50 && r[12] <= 69) || (nbrGravite[1] == 'mineur')){
                res = `Téléconsultation ou médecin généraliste ou visite à domicile </br>
        ...
        }else if(nbrProstic >= 1){
            if(nbrGravite[0] == 0){ 
                res = `Téléconsultation ou médecin généraliste ou visite à domicile </br>
                    Appelez le 141 si une gêne respiratoire ou des difficultés importantes pour s’alimenter ou boire pendant plus de 24h apparaissent.
                    `;
            }else if(nbrGravite[1] == 'mineur'){
                res = `Téléconsultation ou médecin généraliste ou visite à domicile </br>
                    Appelez le 141 si une gêne respiratoire ou des difficultés importantes pour s’alimenter ou boire pendant plus de 24h apparaissent.
                    `;

            }else if(nbrGravite[0] > 1){
                res = `Téléconsultation ou médecin généraliste ou visite à domicile </br>
                    Appelez le 141 si une gêne respiratoire ou des difficultés importantes pour s’alimenter ou boire pendant plus de 24h apparaissent.
                    `;
            }
        }else if(nbrGravite[1] == 'majeur'){
            res = `Appel 141`;
        }
    }
    else if(r[0] == 'oui' && r[2] == 'oui'){
        if(facteurPronostic == 0){
            if(nbrGravite[0] == 0 || nbrGravite[1] == 'mineur')
                    res += `Téléconsultation ou médecin généraliste ou visite à domicile </br>
                        Appelez le 141 si une gêne respiratoire ou des difficultés importantes pour s’alimenter ou boire pendant plus de 24h apparaissent.
                        `;
        }else if(facteurPronostic >=1){
            if(nbrGravite[0] == 0)
                res += `Téléconsultation ou médecin généraliste ou visite à domicile </br>
                    Appelez le 141 si une gêne respiratoire ou des difficultés importantes pour s’alimenter ou boire pendant plus de 24h apparaissent.
                    `;
            if(nbrGravite[0] == 1 &&nbrGravite[1] == 'mineur')
                res += `Téléconsultation ou médecin généraliste ou visite à domicile </br>
                    Appelez le 141 si une gêne respiratoire ou des difficultés importantes pour s’alimenter ou boire pendant plus de 24h apparaissent.
                    `;
            if(nbrGravite[0] > 1 && nbrGravite[1] == 'mineur')
            res += `Appel 141`;
        }else if(nbrGravite[0] >= 1 && nbrGravite[1] == 'majeur')
            res += `Appel 141`;
    }
    else if([r[0], r[2], r[4], r[3]].filter( e => e == 'oui').length == 1){
        if(nbrGravite[0] == 0){
            res += `Votre situation ne relève probablement pas du Covid-19. Consultez votre médecin au moindre doute.`
        }else if(nbrGravite[0] > 0 || facteurPronostic > 0){
            res += `Votre situation ne relève probablement pas du Covid-19. Un avis médical est recommandé. Au moindre doute, appelez le 141.`
        }
    }
    else if([r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9]].filter( e => e == 'oui' || e == 'mal' || e == 'très mal').length == 0){
        res += `Votre situation ne relève probablement pas du Covid-19. 
        ...
        res += `Restez chez vous au maximum en attendant que les symptômes disparaissent. 
